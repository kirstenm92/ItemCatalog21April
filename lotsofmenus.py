...
###Begin inserting information into database
...
session.add(restaurant2)  #staging zone
session.commit()  #now stored in database
...
#session.query(Restaurant).all() will ask session to go into database,
#find table that corresponds to the restaurant class, and find all entries
#in that table and return them in a list

#firstResult=session.query(Restaurant).first()
#this variable corresponds to a single row in my database
#these single row references allow me to extract column entries
#as method names
#e.g. firstResult.name = u'Pizza Palace'
